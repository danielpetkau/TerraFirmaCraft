...
public class FoodBehaviorTests
{
    ...
    public void testCreatingAlmostExpiredFlourIntoDoughDoesNotExpire(GameTestHelper helper)
    {
        ...
        try (final CalendarTransaction tr = Calendars.SERVER.transaction())
        {
            ...
            final CraftingInput input = CraftingInput.of(2, 1, List.of(grainStack, waterStack));

            final RecipeHolder<CraftingRecipe> recipe = helper.getLevel()
                .getRecipeManager()
            ...
            final ItemStack doughStack = recipe.value().assemble(input, ServerLifecycleHooks.getCurrentServer().registryAccess());
